...
from telephone.classes.view_models.SubscribeTransaction import PendingTransactionVM
...
def get_pending_transacts(request, template):
	...
	:param request: HTTP request
	:param template: html template
	:return: HttpResponcse instance
	"""
	pending_transact = [PendingTransactionVM(transact) for transact in SubscribeTransaction.objects.filter(status_id=1).order_by('-creation_date')]
