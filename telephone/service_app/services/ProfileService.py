...
class ProfileService():
	...
	def create_profile(data):
		...
		try:
			...
			user.save()
			user_profile = UserProfile.objects.create(profile_email='%s@%s' % (data['login'], settings.DOMAIN), profile_password=data['password'], uid=data['uid'], token=data['token'], user_key=data['userKey'], secret_key=data['secretKey'], user_id=user.id, customer_number=data['customerNumber'])
			user_profile.save()
